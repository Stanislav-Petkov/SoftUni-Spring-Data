...
public class EntityManager<E> implements DbContext<E> {
    ...
    public boolean persist(E entity) throws IllegalAccessException, SQLException {
        ...
        if (value == null || (int) value <= 0) {
            // if the current user does not have id , insert it in the db
    ...
    private boolean doInsert(E entity, Field primary) throws IllegalAccessException, SQLException {
        if (!this.checkIfTableExists(entity.getClass())) {
            // If the table was not created, create it
            this.doCreate(entity.getClass());
        }else{
            // If the table exists, check if column exists
    ...
    private boolean checkIfTableExists(Class entity) throws SQLException {
        ...
        // If the table was created, rs.next() will return true
        return result;
    }

    public boolean  checkIfColumnExists(Field field) throws SQLException {
        String query =
                "SELECT `COLUMN_NAME`\n" +
